...
export const updateAverageResultsInStore = async (
  gameId: string,
  result: number,
): Promise<boolean> => {
  try {
    const doc = await db.collection(gamesCollectionName).doc(gameId).get();

    if (!doc.exists) {
      throw Error('The current game id does not exist in the database');
    }

    const currentResults = doc.data()?.results || [];
    currentResults.push(result);

    await db.collection(gamesCollectionName).doc(gameId).update({
